<template>
  <div class="p-2 w-100 text-primary">
    <b-row>
      <b-col
      ...
      >
        <b-card no-body class="my-5 w-100 h-100">
          <b-card-header header-tag="header" class="p-0" :class="`bg-primary`">
            <!-- <b-button v-b-toggle="`accordion-${item.id}`" block href="#" variant="primary"> -->
            <b-button block href="#" :to="`/services/${item.id}/`" :variant="`primary`" class="text-medium text-white">
              {{ item.landing.header }}
            </b-button>
          </b-card-header>
          <!-- <b-collapse :id="`accordion-${item.id}`" visible accordion="my-accordion" role="tabpanel"> -->
          <b-card-body>
            <b-card-text class="text-center text-medium">
              <font-awesome-icon :icon="item.icon" :class="`fa text-${item.variant}`" />
            </b-card-text>
            <b-card-text>{{ item.landing.content }}</b-card-text>
            <b-card-text>
              <nuxt-link :to="`/services/${item.id}/`" :class="`text-black`" :variant="`${item.variant}`">
                Find out more
              </nuxt-link>
